...
def series(*resistors):
    """Return equivalent resistance of resistors in parallel."""
    return sum(r for r in resistors)
...
def all_series_combinations(resistors):
    results = []
    n = len(resistors)
    for k in range(1, n + 1):
        for combo in itertools.combinations(resistors, k):
...
combos = all_series_combinations(Capacitors_available)

vals=[]
i=0
for combo, value in combos:
    i=i+1
    print(f"{i}\t{combo}\t{value}")
    vals.append(value)
...
for i in range(len(differences)):
    print(f"{i}\t{sorted_vals[i]:.2f} + {differences[i]:.2f} = {sorted_vals[i+1]:.2f}")

# print(differences)
print(max(differences))

make_tipmix_chart(np.arange(2**4),4)
