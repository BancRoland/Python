...
print(f"MAX VOLTAGE AND CURRENT LIMITATIONS")

for resistor,power_limit in resistors_with_powers:
    max_current = np.sqrt(power_limit/resistor)
    max_voltage = np.sqrt(power_limit*resistor)
    # print(f"R = {resistor} Ohm\tP = {power_limit} W\tU_max = {max_voltage:.3f} V\tI_max = {max_current:.3f} A")
    print(f"R = {resistor:12.6f} Ohm\t"
        f"P = {power_limit:4d} W\t"
        f"U_max = {max_voltage:9.3f} V\t"
        f"I_max = {max_current:7.3f} A")

print(f"\n\nVOLTAGE SET TO FIX, CURRENT TO MAX")

for resistor,power_limit in resistors_with_powers:
    voltage_on_resistor = 30    # V
    current_on_resistor = voltage_on_resistor/resistor
    if current_on_resistor>=3:
        current_on_resistor=3
        voltage_on_resistor=current_on_resistor*resistor
    power_on_resistor = voltage_on_resistor * current_on_resistor
    print(f"R = {resistor} Ohm\tU = {voltage_on_resistor:.3f} V\tI = {current_on_resistor:.3f} A\tP = {power_on_resistor:.3f} / {power_limit} W\t{power_limit>power_on_resistor}")


# for resistor,power_limit in resistors_with_powers:
#     power_on_resistor = 1
#     voltage_on_resistor = (power_on_resistor*resistor)**0.5
#     current_on_resistor = (power_on_resistor/resistor)**0.5
 
#     print(f"R = {resistor} Ohm\tU = {voltage_on_resistor:.3f} V\tI = {current_on_resistor:.3f} A\tP = {power_on_resistor:.3f} / {power_limit} W\t{power_limit>power_on_resistor}")

print(f"CURRENT SET TO FIX, VOLTAGE TO MAX")

for resistor,power_limit in resistors_with_powers:
    current_on_resistor = 0.1
    voltage_on_resistor = resistor*current_on_resistor
    if voltage_on_resistor>30:
        voltage_on_resistor = 30
        current_on_resistor = voltage_on_resistor/resistor
    power_on_resistor = voltage_on_resistor*current_on_resistor
...
print(f"POWER SET TO FIX")

P = 1 # [W]

for resistor,power_limit in resistors_with_powers:
    current_on_resistor = np.sqrt(P/resistor)
    voltage_on_resistor = np.sqrt(P*resistor)
...
for i in range(len(resistors_with_powers)):
    print(f"#{i}:\t{resistors_with_powers[i]}")
...
matches = closest_combinations(resistors_with_powers, Resistors_target)

for m in matches:
    diff_percent=m['difference']/m["target"]*100
    indexes = [resistors_with_powers.index(r) for r in m["combination"]]
    ...
    make_tipmix_chart(indexes,len(resistors_with_powers))
